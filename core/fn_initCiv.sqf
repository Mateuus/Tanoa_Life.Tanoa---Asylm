...
if (worldName != "Tanoa") then
{
	[] spawn
	{
		...
		while {true} do
		{
			...
			waitUntil {sleep 60; _lastRobbed = 0; _cops = 0; { if (side _x == west) then { _cops = _cops + 1; }; } forEach playableUnits; if (!isNil "life_bank_lastRobbed") then { _lastRobbed = life_bank_lastRobbed; }; (!life_bank_inProgress && (player distance (getMarkerPos "fed_reserve") < _distance) && (_lastRobbed < (time - 600)) && _cops > 5)};
