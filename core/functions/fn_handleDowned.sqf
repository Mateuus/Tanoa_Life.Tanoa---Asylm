...
{	
	...
	_curMags = magazines player + [currentMagazine player];
	...
	while {_downed} do {
		...
		if (alive player) then 
		{
			if (!(_inVehicle)) then 
			{
				if (_time == 8) then {[[player,"AinjPpneMstpSnonWnonDnon","playNow"],"life_fnc_animSync",true,false] spawn life_fnc_MP;};
				if((isNull (attachedTo player)) && !(player getVariable ["Escorting",false])) then {
					_obj = "Land_ClutterCutter_small_F" createVehicle ASLTOATL(visiblePositionASL player); 
					_obj setPosATL ASLTOATL(visiblePositionASL player);
					[[player,"AinjPpneMstpSnonWnonDnon","playNow"],"life_fnc_animSync",true,false] spawn life_fnc_MP;
					player attachTo [_obj,[0,0,0]];
					player setVariable["downedObj",_obj,true];
				};
			};
			if ((_vehSource && _time == 16) || (!_vehSource && _time == 60) || player getVariable["adrenaline",false]) then {_downed = false; player setVariable["adrenaline",false]};
		} else 
		{
			_downed = false;
			_dead = true;
	...
	[[player,"AmovPpneMstpSnonWnonDnon","playNow"],"life_fnc_animSync",true,false] spawn life_fnc_MP;
