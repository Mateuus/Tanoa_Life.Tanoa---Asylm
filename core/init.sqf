...
diag_log "------------------------------------------------------------------------------------------------------";
life_ringer = (profileNamespace getVariable["ASYsettings",[1600,1600,1600,true,false]]) select 3;
cutText ["","BLACK IN"];
...
[] spawn
{
	while {true} do
	{
		...
		if (!isNil "license_civ_bounty" && {(player getVariable ["isBountyHunter", false]) != license_civ_bounty}) then
		{
